...
function showFallback(errorDetail) {
  ...
  if (errorDetail) {
    ...
    dbg.style.cssText = 'font-family:monospace;font-size:11px;opacity:0.5;margin-top:24px;';
    dbg.textContent = 'debug: ' + errorDetail;
    fallbackScreen.appendChild(dbg);
  }
}

function initScene() {
  scene = new THREE.Scene();
  scene.fog = new THREE.FogExp2(CONFIG.colors.void, 0.006);

  camera = new THREE.PerspectiveCamera(58, window.innerWidth / window.innerHeight, 0.1, 800);
  camera.position.set(0, 0, 9);

  ambientLight = new THREE.AmbientLight(new THREE.Color(CONFIG.colors.mutedPurple), 0.7);
  scene.add(ambientLight);
  warmLight = new THREE.PointLight(new THREE.Color(CONFIG.colors.gold), 0.6, 60);
  warmLight.position.set(6, 4, 8);
  scene.add(warmLight);
  const pinkLight = new THREE.PointLight(new THREE.Color(CONFIG.colors.blushPink), 0.5, 60);
...
function buildStarfield() {
  ...
  const pTex = makeRadialTexture(hexToCss(CONFIG.colors.blushPink, 1), hexToCss(CONFIG.colors.blushPink, 0));
  const pMat = new THREE.PointsMaterial({
    size: 1.6, map: pTex, transparent: true, opacity: 0.8,
...
function buildNebula() {
  ...
  const configs = [
    { color: hexToCss(CONFIG.colors.mutedPurple, 0.85), pos: [-14, 6, -22], scale: 42 },
    { color: hexToCss(CONFIG.colors.blushPink, 0.55), pos: [16, -6, -28], scale: 36 },
    { color: hexToCss(CONFIG.colors.midnightNavy, 0.8), pos: [0, 14, -38], scale: 50 },
    { color: hexToCss(CONFIG.colors.blushPink, 0.42), pos: [-20, -10, -18], scale: 30 },
    { color: hexToCss(CONFIG.colors.gold, 0.28), pos: [10, 10, -14], scale: 24 },
    { color: hexToCss(CONFIG.colors.blushPink, 0.35), pos: [0, -16, -34], scale: 34 },
  ];
  configs.forEach(cfg => {
...
function buildPhotos() {
  // group photos that share a "pair" tag so they float near each other
  const pairCenters = {};
  let soloIndex = 0;
  const soloCount = CONFIG.photos.filter(p => !p.pair).length;

  CONFIG.photos.forEach((photo, i) => {
    let basePos;
    if (photo.pair) {
      if (!pairCenters[photo.pair]) {
        const seed = Object.keys(pairCenters).length;
        const angle = (seed / Math.max(1, Object.keys(pairCenters).length + 2)) * Math.PI * 2 + 0.8;
        const radius = 8.5;
        pairCenters[photo.pair] = {
          center: new THREE.Vector3(Math.cos(angle) * radius, Math.sin(seed * 1.7) * 3, Math.sin(angle) * radius - 2),
          count: 0
        };
      }
      const p = pairCenters[photo.pair];
      const offset = p.count === 0 ? new THREE.Vector3(-0.85, 0.35, 0.4) : new THREE.Vector3(0.85, -0.35, -0.4);
      basePos = p.center.clone().add(offset);
      p.count++;
    } else {
      const angle = (soloIndex / Math.max(1, soloCount)) * Math.PI * 2 + 2.6;
      const radius = 10.5 + (soloIndex % 2) * 2.5;
      basePos = new THREE.Vector3(Math.cos(angle) * radius, Math.sin(soloIndex * 1.7) * 3, Math.sin(angle) * radius - 2);
      soloIndex++;
    }
    ...
    group.userData = {
      baseY: group.position.y, phase: Math.random() * Math.PI * 2,
      rotSpeed: (Math.random() - 0.5) * 0.15,
      kind: 'photo', basePos: group.position.clone(), caption: photo.caption || ''
    };
    scene.add(group);
    photoGroup.push(group);
    if (photo.caption) interactiveGroup.push(group);
  });
}

/* --- little floating surprises (interactive objects) --- */
function makeGlowSprite(colorHex, size) {
  const tex = makeRadialTexture(hexToCss(colorHex, 1), 'rgba(0,0,0,0)');
  const mat = new THREE.SpriteMaterial({ map: tex, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending });
  const sprite = new THREE.Sprite(mat);
...
function buildSurpriseMesh(type) {
  ...
  group.add(core);
  group.add(makeGlowSprite(type === 'star' ? CONFIG.colors.blushPink : CONFIG.colors.gold, 3.6));
  return group;
}
...
const controlState = {
  radius: 9, theta: 0, phi: Math.PI / 2.1,
  targetTheta: 0, targetPhi: Math.PI / 2.1, targetRadius: 9,
...
async function onObjectActivated(obj) {
  ...
  else if (kind === 'photo') await activatePhoto(obj);
}

async function activatePhoto(obj) {
  const pos = worldPos(obj);
  const dir = pos.clone().normalize();
  const camTarget = pos.clone().sub(dir.multiplyScalar(3.5));
  await flyCameraTo(camTarget, pos, 1600);
  showMessageCard(obj.userData.caption);
  markDiscovered('photo-' + (obj.userData.caption || Math.random()));
}
...
(function boot() {
  let ok = false;
  try { ok = initRenderer(); } catch (e) { showFallback(e.message); return; }
